...
---
Population Genetics in R Handbook was referenced (https://grunwaldlab.github.io/Population_Genetics_in_R/index.html). 
A CSV file is also needed with data on each isolate, including sample names, location, latitude and longitude coordinates for the map, and any other data that is of interest in the analysis. 
...
clean=read.vcfR("~/data_and_metadata//filteredpfulvanodup.vcf.gz")
...
cleancc=read.vcfR("~/Data_and_metadata//cleanclonepfulva.out.vcf.gz")
...
#load sample info for clone-corrected metadataset
...
```{r, eval=TRUE, include=TRUE}
#nj tree
tiff("nj1000.tiff", units="in", width=10, height=6, res = 600)
set.seed(9)
tree <- aboot(gbs.pass, tree = "nj", distance = bitwise.dist, showtree = T, sample = 1000, cutoff = 50, quiet = T, root=TRUE)
...
tiff("mltree_pfulva.tiff", units="in", width=8, height=8, res = 600)
gg <- ggtree(iqtree)
gg <- gg %<+% metadata
...
Principal Component Analysis with clone-corrected metadataset
```{r, eval=TRUE, include=TRUE}
...
tiff("pca_1and2.tiff", units="in", width=5, height=4.5, res=600)
set.seed(9)
...
tiff("pca_1and3.tiff", units="in", width=5, height=4.5, res=600)
set.seed(9)
...
tiff("pca_2and3.tiff", units="in", width=5, height=4.5, res=600)
set.seed(9)
...
Nei Gst, Hedrick G'st with clone corrected metadataset
```{r, eval=TRUE, include=TRUE}
